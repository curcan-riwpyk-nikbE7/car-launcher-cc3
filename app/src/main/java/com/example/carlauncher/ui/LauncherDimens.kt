...
fun calcDimens(w: Dp, h: Dp): LauncherDimens {
    ...
    // Полоса статуса занимает свою высоту над карточками. 6.5% от 720
    // это 47 px: значки становятся заметно крупнее и по ним проще
    // попасть пальцем на ходу. Карточки теряют около десятка пикселей —
    // приемлемая плата за читаемость.
    val statusH = (hv * 0.065f).coerceIn(30f, 48f)
